...
# Задание размеров экрана создания подписанного сертификата
S_FRAME_WIDTH = 600
S_FRAME_HEIGHT = 350
S_ENTRY_WIDTH = 70


# Класс экрана с настройкой параметров для создания подписанного сертификата
class SignedSslFrame:
    def __init__(self, gui):
        self.master = gui.master
        self.gui = gui

        self.frame = Frame(self.master, width=S_FRAME_WIDTH, height=S_FRAME_HEIGHT)
        self.option_boxes = []

        # Пары подпись и начальные значения параметров сертификата
        option_texts = {
            'CA key path': 'D:\\SUSU\\Practice\\Безопасность информационных систем\\ssl_generation\\cert\\CA_KEY.key',
            'CA cert path': 'D:\\SUSU\\Practice\\Безопасность информационных систем\\ssl_generation\\cert\\CA_CRT.crt',
            'Country': 'RU', 'State or province': 'Russia', 'Locality': 'Chelyabinsk',
    ...
    def show(self):
        self.frame.grid()
        self.master.geometry('{0}x{1}'.format(S_FRAME_WIDTH, S_FRAME_HEIGHT))
    ...
    def __create_option_box(self, lbl_text, entry_default_text):
        ...
        entry_option = Entry(self.frame, width=S_ENTRY_WIDTH)
    ...
    def __generate_certificate(self):
        ...
        success = cert_gen.create_signed_cert()
